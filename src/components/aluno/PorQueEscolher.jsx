import React from 'react';

const PorQueEscolher = () => {
  return (
    <section id="por-que-escolher-section" className="py-20 md:py-32 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <h2 className="text-4xl md:text-5xl font-bold text-secondary mb-4">
            Por que escolher nossa plataforma?
          </h2>
          <p className="text-xl text-gray-600 max-w-3xl mx-auto">
            A melhor forma de praticar e melhorar suas habilidades ao volante
          </p>
        </div>

        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          <div className="flex items-start gap-4 p-6 bg-accent rounded-2xl hover:shadow-lg transition-shadow">
            <div className="bg-green-100 rounded-full p-3 flex-shrink-0">
              <svg className="w-6 h-6 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
            </div>
            <div>
              <h4 className="font-bold text-lg text-secondary mb-2">100% Gratuito</h4>
              <p className="text-gray-600">A plataforma é totalmente gratuita. Sem taxas de cadastro ou mensalidade.</p>
            </div>
          </div>

          <div className="flex items-start gap-4 p-6 bg-accent rounded-2xl hover:shadow-lg transition-shadow">
            <div className="bg-blue-100 rounded-full p-3 flex-shrink-0">
              <svg className="w-6 h-6 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" />
              </svg>
            </div>
            <div>
              <h4 className="font-bold text-lg text-secondary mb-2">Instrutores Certificados</h4>
              <p className="text-gray-600">Todos os instrutores são verificados e certificados pelo Detran.</p>
            </div>
          </div>

          <div className="flex items-start gap-4 p-6 bg-accent rounded-2xl hover:shadow-lg transition-shadow">
            <div className="bg-purple-100 rounded-full p-3 flex-shrink-0">
              <svg className="w-6 h-6 text-purple-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4" />
              </svg>
            </div>
            <div>
              <h4 className="font-bold text-lg text-secondary mb-2">Flexibilidade de Veículo</h4>
              <p className="text-gray-600">As aulas podem ser feitas no carro do instrutor ou no seu próprio veículo.</p>
            </div>
          </div>

          <div className="flex items-start gap-4 p-6 bg-accent rounded-2xl hover:shadow-lg transition-shadow">
            <div className="bg-yellow-100 rounded-full p-3 flex-shrink-0">
              <svg className="w-6 h-6 text-yellow-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
            </div>
            <div>
              <h4 className="font-bold text-lg text-secondary mb-2">Horários Flexíveis</h4>
              <p className="text-gray-600">Escolha o horário que melhor se encaixa na sua rotina.</p>
            </div>
          </div>

          <div className="flex items-start gap-4 p-6 bg-accent rounded-2xl hover:shadow-lg transition-shadow">
            <div className="bg-green-100 rounded-full p-3 flex-shrink-0">
              <svg className="w-6 h-6 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6V4m0 2a2 2 0 100 4m0-4a2 2 0 110 4m-6 8a2 2 0 100-4m0 4a2 2 0 110-4m0 4v2m0-6V4m6 6v10m6-2a2 2 0 100-4m0 4a2 2 0 110-4m0 4v2m0-6V4" />
              </svg>
            </div>
            <div>
              <h4 className="font-bold text-lg text-secondary mb-2">Aulas Personalizadas</h4>
              <p className="text-gray-600">Aulas adaptadas ao seu nível e necessidades específicas.</p>
            </div>
          </div>

          <div className="flex items-start gap-4 p-6 bg-accent rounded-2xl hover:shadow-lg transition-shadow">
            <div className="bg-red-100 rounded-full p-3 flex-shrink-0">
              <svg className="w-6 h-6 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
              </svg>
            </div>
            <div>
              <h4 className="font-bold text-lg text-secondary mb-2">Suporte Dedicado</h4>
              <p className="text-gray-600">Atendimento rápido via WhatsApp sempre que precisar.</p>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
};

export default PorQueEscolher;

